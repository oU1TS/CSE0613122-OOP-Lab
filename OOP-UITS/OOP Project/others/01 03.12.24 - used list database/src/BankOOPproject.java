...
class SavingsAccount extends Account implements Authenticatable {
    ...
    void initializeSavings(String name, String password, int initialBalance) {
        super.initialize(name, "Savings", initialBalance);
...
public class BankOOPproject {
        ...
        while (!quit) {
            try {
                ...
                switch (choice) {
                    ...
                    case 4: {
                        Account account = authenticateAccount(accounts, in);
                        if (account != null) {
    ...
    private static Account authenticateAccount(List<Account> accounts, Scanner in) {
        Account account = findAccount(accounts, in);
        ...
        if (account instanceof Authenticatable) {
            ...
            String password = in.next();
            if (!((Authenticatable) account).authenticate(password)) {
